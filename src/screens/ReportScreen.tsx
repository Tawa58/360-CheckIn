...
const STATUS_STYLE = {
  ...
  seen: {bg: 'bg-sky-50', text: 'text-sky-700', label: 'Seen'},
    ...
    <SafeAreaView className="flex-1 bg-ink-100 dark:bg-slate-950">
      <KeyboardAvoidingView
        ...
        <ScrollView
            ...
            {TABS.map(tab => (
              <Pressable
                ...
                className={`flex-1 rounded-xl px-3 py-2.5 ${
                  kind === tab.id ? 'bg-brand-800' : 'bg-transparent'
                }`}>
                <Text
                  className={`text-center text-sm font-semibold ${
                    kind === tab.id ? 'text-white' : 'text-ink-500'
                  }`}>
                  {tab.label}
                </Text>
              </Pressable>
          ...
          <Text className="mt-3 text-sm text-ink-500">{activeTab?.hint}</Text>

          {kind === 'absence' ? (
            <View className="mt-5">
              <Text className="mb-2 text-sm font-semibold text-ink-800">Date away</Text>
              <View className="flex-row items-center rounded-2xl border border-ink-200 bg-white px-3 py-2">
                <Pressable onPress={() => shiftAbsenceDate(-1)} className="h-10 w-10 items-center justify-center">
                  <Text className="text-xl font-bold text-brand-800">‹</Text>
                </Pressable>
            ...
            {categories.map(item => (
              <Pressable
                ...
                className={`rounded-full px-3 py-1.5 ${
                  category === item ? 'bg-brand-800' : 'bg-white'
                }`}>
                <Text
                  className={`text-xs font-semibold ${
                    category === item ? 'text-white' : 'text-ink-600'
                  }`}>
                  {item}
                </Text>
              </Pressable>
          ...
          <Text className="mb-2 mt-5 text-sm font-semibold text-ink-800">Details</Text>
          <TextInput
            multiline
            textAlignVertical="top"
            placeholderTextColor="#8A9AA8"
            className="min-h-[120px] rounded-2xl border border-ink-200 bg-white px-4 py-3 text-base text-ink-900"
            placeholder={
              kind === 'absence'
          ...
          <Text className="mb-3 mt-8 text-lg font-semibold text-ink-900">Your reports</Text>
          {loading ? (
            <Text className="text-sm text-ink-500">Loading reports…</Text>
          ) : messages.length === 0 ? (
            <Text className="text-sm text-ink-500">You have not sent a report yet.</Text>
          ) : (
              ...
              return (
                <View key={message.messageId} className="mb-3">
                  <Card>
                    ...
                    <Text className="mt-2 text-sm text-ink-600">{message.details}</Text>
